			...
			<div class="p-4 card-body">
				<div class="text-center">
					<div class="mx-auto mb-4 profile-user position-relative d-inline-block">
						<img src="https://www.casperproject.my.id/storage/profile/default.jpg" class="rounded-circle avatar-xl img-thumbnail user-profile-image material-shadow" alt="user-profile-image">
					</div>
						...
						<form action="{{ route(strtolower(auth()->user()->role->name) . '.profile.update', auth()->user()->id) }}" method="POST">
							...
							<div class="row">
								<div class="col-lg-6">
									...
									<div class="col-lg-6">
										<div class="mb-3">
											<x-input label="Identity"  type="text" name="identity" value="{{ auth()->user()->identity }}" attr="readonly"/>
											</div>
										</div>
										<div class="col-lg-6">
											<div class="mb-3">
												<x-input label="Email" type="email" name="email" value="{{ auth()->user()->email }}" :isRequired="false"/>
												</div>
											</div>
											<div class="col-lg-6">
												<div class="mb-3">
												...
												<div class="col-lg-6">
													<div class="mb-3">
														<x-select name="gender" label="Gender" id="gender_update" :options="$gender"  selected="{{ auth()->user()->gender }}"/>  
														</div>
													</div>
													<div class="col-lg-6">
														<div class="mb-3">
															<x-select name="program_study" label="Program Study" id="program_study" :options="$programStudy" selected="{{ auth()->user()->program_study_id }}" /> 
															</div>
														</div>
